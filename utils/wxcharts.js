...
var config = {
    ...
    columePadding: 3,
    fontSize: 12,
    dataPointShape: ['circle', 'circle', 'circle', 'rect'],
...
function getYOnCanvas(data, minRange, maxRange, opts, config) {
    var y = 0;
    var validHeight = opts.height - 2 * config.padding - config.xAxisHeight - config.legendHeight;
    var height = validHeight * (data - minRange) / (maxRange - minRange);
    y = opts.height - config.xAxisHeight - config.legendHeight - Math.round(height) - config.padding;
...
function drawPointText(points, series, config, context) {
    ...
    context.setFillStyle(series.pointColor[0]);
    points.forEach(function (item, index) {
        if (item != null) {
                ...
                onProcess: function onProcess(process) {
                    ...
                    if(dashedLine) {
                        ...
                        dashedLine.forEach(function(item, index){
                            var startY = getYOnCanvas(item.startY, minRange, maxRange, opts, config);
                            var endY = getYOnCanvas(item.endY, minRange, maxRange, opts, config)
                            dashedLineTodashedLineTo(context, item.endX, endY, item.startX, startY, item.lineLength, item.color);
                        })
                    }
                },
